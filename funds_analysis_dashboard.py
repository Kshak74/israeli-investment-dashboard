...
def create_dashboard(df):
    ...
    with tab1:
        ...
        with col1:
            ...
            fig_geo_bar.update_traces(marker_line_width=0, width=0.85)  # עיבוי שנתות
    ...
    with tab2:
        ...
        with col1:
            ...
            fig_strat_bar.update_traces(marker_line_width=0, width=0.85)  # עיבוי שנתות
    ...
    with tab6:
        ...
        if uploaded_files and len(uploaded_files) > 1:
            ...
            # --- ודא שכל הערכים חד-ממדיים (str) ---
            for col in ['Period', 'Geography', 'Strategy']:
                if col in all_data.columns:
                    all_data[col] = all_data[col].astype(str)
            # סדר כרונולוגי: סדר לפי סדר קבצים (או אלפביתית אם תרצה)
            period_order = period_labels
            all_data['Period'] = pd.Categorical(all_data['Period'], categories=period_order, ordered=True)
